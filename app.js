...
async function init() {
  ...
  initBattle();
  renderAll();
  log("準備完了。敵HPを3倍にし、水上由岐の画像をR2から読み込みます。");
}
...
function spriteContent(card, kind="battle") {
  let url = "";
  if (kind === "icon") {
    url = card.iconImage || card.sdImage || card.battleImage || "";
  } else if (kind === "sd") {
    url = card.sdImage || card.battleImage || card.iconImage || "";
  } else {
    url = card.battleImage || card.sdImage || card.iconImage || "";
  }
  if (url) {
...
function spriteSheetStyle(card, frameIndex) {
  ...
  const fw = card.frameWidth || 280;
  const fh = card.frameHeight || 280;
  const col = frameIndex % cols;
  const row = Math.floor(frameIndex / cols);
  const x = -(col * fw);
  const y = -(row * fh);
  return `background-image:url('${card.sdSheetImage}');background-size:${fw * cols}px ${fh * rows}px;background-position:${x}px ${y}px;`;
}
...
function renderField() {
  ...
  battle.allies.forEach((u, i) => {
    if (u.alive) field.appendChild(createUnitEl(u, i, "ally"));
  });
  battle.enemies.forEach((u, i) => {
    if (u.alive) field.appendChild(createUnitEl(u, i, "enemy"));
  });
}
...
function renderUltimate() {
  ...
  battle.allies.forEach(u => {
    ...
    btn.className = `ultIcon ${u.tp >= 100 ? "ready" : ""}`;
    btn.innerHTML = `
      <div class="iconFace">${spriteContent(u.card, "icon")}</div>
